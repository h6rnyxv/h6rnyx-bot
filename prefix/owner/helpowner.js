...
export default {
  ...
  async ejecutar({ client, message }) {
    ...
    const embed5 = new EmbedBuilder()
      .setTitle('🤖 Panel del Owner — Control de Selfbots')
      .setColor('DarkRed')
      .addFields(
        {
          name: '📊 Estado',
          value: [
            '`!status` — estado detallado de todas las cuentas selfbot',
            '`!cuentas` — lista rápida de cuentas conectadas',
          ].join('\n'),
        },
        {
          name: '💬 Mensajes (todas las cuentas)',
          value: [
            '`!sayall <msg>` — todas las cuentas envían el mensaje en el canal actual',
            '`!spamall <1-30> <msg>` — todas hacen spam en el canal actual',
          ].join('\n'),
        },
        {
          name: '🎭 Presencia (todas las cuentas)',
          value: [
            '`!statusall <online|idle|dnd|invisible>` — cambia el estado de todas',
            '`!actividadall [texto]` — cambia la actividad de todas (vacío para quitar)',
            '`!avatarall <url>` — cambia el avatar de todas con la misma imagen',
            '`!hypesquadall <bravery|brilliance|balance>` — HypeSquad de todas',
            '`!joinacc <enlace>` — todas las cuentas se unen al servidor',
          ].join('\n'),
        },
        {
          name: '🖼️ Individual — Avatar',
          value: [
            '`!setavatar <num> <url>` — cambia el avatar de esa cuenta',
            '`!setavatar 1 <url1> 2 <url2>` — avatares distintos por cuenta',
          ].join('\n'),
        },
        {
          name: '🎭 Individual — Presencia & Perfil',
          value: [
            '`!setstatus <num> <online|idle|dnd|invisible>` — estado de esa cuenta',
            '`!setactividad <num> <texto>` — actividad de esa cuenta',
            '`!setdesc <num> <texto>` — bio de esa cuenta',
            '`!cleardesc <num>` — elimina la bio de esa cuenta',
          ].join('\n'),
        },
        {
          name: '💬 Individual — Mensajes',
          value: '`!accsay <num> <channelId|userId> <texto>` — esa cuenta envía el mensaje a un canal o MD',
        },
      )
      .setFooter({ text: 'Selfbot Controller • Prefix: !' });
