...
export default {
  data: new SlashCommandBuilder()
    ...
    .addStringOption(opt =>
      opt.setName('etiqueta')
        .setDescription('Etiqueta para identificar la key (opcional)')
        .setRequired(false)
    ),
  ...
  async execute(interaction) {
    ...
    if (!adminKey) {
      return interaction.reply({ content: '❌ `ADMIN_KEY` no configurada en el bot.', ephemeral: true });
    }
    ...
    try {
      ...
      if (!res.ok || !data.key) {
        return interaction.editReply({ content: `❌ Error: ${data.error || 'No se pudo generar la key.'}` });
      }
      ...
      const embed = new EmbedBuilder()
        ...
        .setFooter({ text: `Generada por ${interaction.user.tag}` })
        .setTimestamp();
