...
export default {
  data: new SlashCommandBuilder()
    .setName('revokekey')
    .setDescription('[Admin] Revoca una key existente')
    .addStringOption(opt =>
      opt.setName('key')
        .setDescription('La key a revocar')
        .setRequired(true)
    ),
    ...
    try {
      ...
      if (!res.ok || data.error) {
        return interaction.editReply({
          embeds: [
            new EmbedBuilder()
              .setTitle('❌ Error al revocar')
              .setDescription(data.error || 'No se pudo revocar la key.')
              .setColor(0xed4245),
          ],
        });
      }
      ...
      const embed = new EmbedBuilder()
        .setTitle('🗑️ Key Revocada')
        .setColor(0xfee75c)
        .setDescription(`La key \`${key}\` fue revocada exitosamente.`)
        .setFooter({ text: `Revocada por ${interaction.user.tag}` })
        .setTimestamp();
