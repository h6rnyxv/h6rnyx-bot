import { EmbedBuilder } from 'discord.js';
  ...
  export async function sendKeyLog(client, guildId, { accion, ejecutadoPor, key, tipo, expira, extra }) {
    ...
    const colores = { genkey: 0x57f287, revokekey: 0xed4245, checkkey: 0x5865f2 };
    const iconos  = { genkey: '🔑', revokekey: '🗑️', checkkey: '🔍' };

    const embed = new EmbedBuilder()
      .setTitle(`${iconos[accion] || '📋'} Log de Key — ${accion}`)
      .setColor(colores[accion] || 0x99aab5)
      .addFields({ name: '👤 Ejecutado por', value: ejecutadoPor, inline: true })
